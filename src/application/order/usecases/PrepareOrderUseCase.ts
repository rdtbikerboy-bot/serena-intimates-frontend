...
export class PrepareOrderUseCase {
  ...
  async execute(orderId: string, note?: string): Promise<Order> {
    ...
    order.transitionTo(OrderStatus.Preparing, note, 'ADMIN');
    serenaLogger.info('Order prepared', { orderId });
